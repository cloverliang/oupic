...
def fit_sigma_lambda_estimator(exp_data: pd.DataFrame, tree: dp.Tree) :
    ...
    # euclidean distance
    exp_dist_df = pd.DataFrame(squareform(pdist(exp_data.T, 'sqeuclidean') ), columns=exp_species, index=exp_species) / ngene
    ...
    return { 'OptimizedValue': res, 'hessian':res_hessian, 'xtrain':xtrain, 'ytrain':ytrain }