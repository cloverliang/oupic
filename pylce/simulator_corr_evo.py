...
def cov_ou(sigma_x, sigma_y, lambda_x, lambda_y, t_x, t_y, gamma_xy):
    return (
        ...
        * sigma_y
        / (lambda_x + lambda_y)
        * np.exp(-lambda_x * t_x - lambda_y * t_y)
    )
...
def calc_sigma_x_ou(tree, attr_x):
    ...
    for taxon_x in tree.taxon_namespace:
        for taxon_y in tree.taxon_namespace:
            ...
            sigma_mtx.loc[taxon_x.label, taxon_y.label] = cov_ou(
                sigma_x, sigma_x, lambda_x, lambda_x, t_x, t_y, 1
            )
    return sigma_mtx
...
def calc_sigma_xy_ou(tree, attr_xy):
    ...
    for i, taxon_a in enumerate(tree.taxon_namespace):
        for j, taxon_b in enumerate(tree.taxon_namespace):
            ...
            sigma_xy_C[i, j] = cov_ou(
                sigma_y, sigma_x, lambda_y, lambda_x, t_a, t_b, gamma_xy
            )
            sigma_xy_D[i, j] = cov_ou(
                sigma_x, sigma_y, lambda_x, lambda_y, t_a, t_b, gamma_xy
            )

    # merge to sigma_xy
    sigma_xy = pd.DataFrame(
        np.block([[sigma_xy_A, sigma_xy_D], [sigma_xy_C, sigma_xy_B]])
    )
    sigma_xy.columns = [nd.label + "_x" for nd in tree.taxon_namespace] + [
        nd.label + "_y" for nd in tree.taxon_namespace
    ]
    sigma_xy.index = [nd.label + "_x" for nd in tree.taxon_namespace] + [
        nd.label + "_y" for nd in tree.taxon_namespace
    ]
...
def calc_sigma_xy_bm(tree, attr_xy):
    ...
    for i, nd_a in enumerate(tree.leaf_node_iter()):
        for j, nd_b in enumerate(tree.leaf_node_iter()):
            dist_to_root = 0.5 * (
                nd_a.distance_from_root()
                + nd_b.distance_from_root()
                - pdm(nd_a.taxon, nd_b.taxon)
            )
    ...
    sigma_xy.columns = [nd.label + "_x" for nd in tree.leaf_node_iter()] + [
        nd.label + "_y" for nd in tree.leaf_node_iter()
    ]
    sigma_xy.index = [nd.label + "_x" for nd in tree.leaf_node_iter()] + [
        nd.label + "_y" for nd in tree.leaf_node_iter()
    ]
