...
class LCEEstimator:
    ...
    def fit_mle_estimator(self, y0: float, y1: float) -> float:
        ...
        """
        def minus_log_likelihood(rho: float): # a function that takes rho and returns a minus likelihood
            return - self.get_log_likelihood(rho, y0, y1)
        
        res = minimize(minus_log_likelihood, 0., method='L-BFGS-B', bounds=[(-0.99, 0.99)])
        return res.x
    ...
    def get_posterior_pdf(self, y0: float,
                           y1: float) -> Callable[[np.ndarray], np.ndarray]:
        """
        Get P(rho|y0,y1) as a function (Callable)
        """
        def func(rho):
            post_pdf = self._normal_density(rho, y0,
                                            y1) * self._prior_beta_density(rho)
            return post_pdf
    ...
    def get_log_likelihood(self, rho: float, y0: float, y1:float) -> np.ndarray:
        ...
        """
        return np.log(self._normal_density(rho, y0, y1)) + np.log(self._prior_beta_density(rho))
    ...
    def _normal_density(self, rho: float, y0: float, y1: float) -> np.ndarray:
        ...
        return np.exp(-0.5 * (y0**2 + y1**2 - 2 * y0 * y1 * rho) /
                      root_det**2) / (2 * np.pi * root_det)
    ...
    def _log_likelihood_hardcode(self, rho: float, y0: float,
                                 y1: float) -> np.ndarray:
        """
        Get log likelihood (proportional) by hard code
        """
        return {(self.alpha - 3 / 2) * np.log(1 + rho) +
                (self.beta - 3 / 2) * np.log(1 - rho) - 1 / 4 / self.omega *
                (y0**2 + y1**2 - 2 * y0 * y1 * rho) / (1 - rho**2)}
